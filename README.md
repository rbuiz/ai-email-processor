...
```mermaid
graph TD
    A[✉️ Email Trigger IMAP<br>читает письма] --> B(🛠️ Code in JavaScript<br>чистит текст от кода);
    B --> C{🤖 AI Agent<br>GPT-4.1 Mini<br>анализ, JSON-отчет};
    ...
    H --> I[📊 Append row in sheet<br>запись идей в таблицу];
    H --> J[📄 Create a document<br>создание документа];
    J --> K[📄 Update a document<br>запись выжимки в документ];
```
